...
def safe_predict(learner, img_path):
    ...
    try:
        # Try different approaches to prediction
        
        # Method 1: Direct path prediction
        try:
            pred_class, pred_idx, outputs = learner.predict(img_path)
        except:
            # Method 2: Load image first, then predict
            try:
                from fastai.vision.all import PILImage
                pil_img = PILImage.create(img_path)
                pred_class, pred_idx, outputs = learner.predict(pil_img)
            except:
                # Method 3: Use Path object
                try:
                    path_obj = Path(img_path)
                    pred_class, pred_idx, outputs = learner.predict(path_obj)
                except:
        ...
        st.write(f"Debug - pred_idx: {pred_idx}")
